...
// Shoemaker's deCasteljau algorithm
void sdeCasteljau(double knot_seq[], int size)
{
  //double delta_prev, delta_curr, delta_next;
  double* x, y, z;
  x = new double[size];
  y = new double[size];
  z = new double[size];
  ...
  for (int i=1; i<size-2; ++i)
    {
      double delta_prev = knot_seq[i] - knot_seq[i-1];
      double delta_curr = knot_seq[i+1] - knot_seq[i];
      double delta_next = knot_seq[i+2] - knot_seq[i+1];
...
void keyboard(unsigned char key, int x, int y)
{
  ...
  //TODO: create more points in the sphere
