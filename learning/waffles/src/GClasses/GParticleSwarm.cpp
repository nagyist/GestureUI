...
GParticleSwarm::GParticleSwarm(GTargetFunction* pCritic, int nPopulation, double dMin, double dRange, GRand* pRand)
...
GRubberBallSwarm::GRubberBallSwarm(int dims, int ballCount, GRand* pRand)
 : m_balls(3 * dims + NUMBER_OF_DOUBLES_TO_CONTAIN_STATS), m_pRand(pRand), m_dims(dims), m_currentBall(0), m_compareBall(0), m_condemnedBall(0)
{
...
void GRubberBallSwarm::advanceBall(double* pBallPos)
{
	...
	for(i = 0; i < m_dims; i++)
	{
		m_pTemp[i] = 1.0 / MAX(1e-9, ABS(m_pNormal[i]));
		sum += m_pTemp[i];
	}
