...
GMixtureOfGaussians::GMixtureOfGaussians(int nKernelCount, GData* pData, int nAttribute, double minVariance, GRand* pRand)
{
...
// static
GMixtureOfGaussians* GMixtureOfGaussians::stochasticHammer(int nMinKernelCount, int nMaxKernelCount, int nItters, int nTrials, GData* pData, int nAttribute, double minVariance, GRand* pRand)
{
...
void GMixtureOfGaussians::test()
{
	...
	for(i = 0; i < KERNEL_COUNT; i++)
	{
		...
		{
			if(		ABS(mean - params[2 * j]) < TOLERANCE &&
					ABS(sqrt(var) - params[2 * j + 1]) < TOLERANCE &&
					ABS(weight - cat.likelihood(j)) < TOLERANCE)
				break;
		}
		if(j >= KERNEL_COUNT)
			ThrowError("Failed");
