...
public:
	...
	// built-in functions
	static double _abs(vector<double>& params) { return ABS(params[0]); }
		...
		if(m_pFunction->IsStub())
		{
			...
			if(pFunc->m_expectedParams >= 0)
			{
				...
				if((int)m_children.size() != pFunc->m_expectedParams)
					ThrowError("The function ", pStub->m_name.c_str(), " expects ", gformat(pFunc->m_expectedParams), " parameters. (Got ", gformat(m_children.size()), ").");
			}
			else
			{
				// A minimum number of parameters is expected
				if((int)m_children.size() < -pFunc->m_expectedParams - 1)
					ThrowError("The function ", pStub->m_name.c_str(), " expects at least ", gformat(-pFunc->m_expectedParams - 1), " parameters. (Got ", gformat(m_children.size()), ".)");
			}
		...
		else if(nests == 0)
		{
			...
			else
			{
				...
				else if(c == '+' || c == '-')
				{
					...
					ThrowError("Unrecognized operator: ", gformat(c));
			}
		}
	}
	return token;
...
{
	{
		...
		if(ABS(y - GMath::logistic(x)) > 1e-12)
