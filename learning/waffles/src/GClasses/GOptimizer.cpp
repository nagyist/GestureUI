...
GTargetFunction::GTargetFunction(int dims)
{
...
double GOptimizer::searchUntil(int nBurnInIterations, int nIterations, double dImprovement)
{
	int i;
	for(i = 1; i < nBurnInIterations; i++)
		iterate();
	double dPrevError;
...
GParallelOptimizers::GParallelOptimizers(int dims)
{
	if(dims > 0)
		m_pRelation = new GUniformRelation(dims, 0);
...
double GParallelOptimizers::searchUntil(int nBurnInIterations, int nIterations, double dImprovement)
{
	int i;
	for(i = 1; i < nBurnInIterations; i++)
		iterateAll();
...
void GActionPath::doAction(int nAction)
{
...
void GActionPath::path(int nCount, int* pOutBuf)
{
