...
public:
	...
	virtual double computeErrorOnline(const double* pVector, unsigned int nPattern)
	{
...
public:
	...
	double searchUntil(int nBurnInIterations, int nIterations, double dImprovement);
};
...
public:
	...
	int length() { return m_nPathLen; }

	/// Gets the first nCount actions of the specified path
	void path(int nCount, int* pOutBuf);
...
public:
	...
	inline int actionCount() { return m_nActionCount; }
