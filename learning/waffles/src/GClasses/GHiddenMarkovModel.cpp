...
void GHiddenMarkovModel::test()
{
	...
	// Check the results
	if(ABS(pInitial[0] - 0.29849966020178786) > 1e-12)
		ThrowError("wrong");
	if(ABS(pInitial[1] - 0.70150033979821202) > 1e-12)
		ThrowError("wrong");
	if(ABS(pTrans[0] - 0.40399443180411687) > 1e-12)
		ThrowError("wrong");
	if(ABS(pTrans[1] - 0.59600556819588313) > 1e-12)
		ThrowError("wrong");
	if(ABS(pTrans[2] - 0.61206063945288547) > 1e-12)
		ThrowError("wrong");
	if(ABS(pTrans[3] - 0.38793936054711436) > 1e-12)
		ThrowError("wrong");
	if(ABS(pSym[0] - 0.49547467745041401) > 1e-12)
		ThrowError("wrong");
	if(ABS(pSym[1] - 0.50452532254958593) > 1e-12)
		ThrowError("wrong");
	if(ABS(pSym[2] - 0.19935077334351725) > 1e-12)
		ThrowError("wrong");
	if(ABS(pSym[3] - 0.80064922665648264) > 1e-12)
		ThrowError("wrong");
}
#endif
