...
{
	int nRegion = (int)m_regions.size();
...
int GRegionAjacencyGraph::regionCount()
{
	return (int)m_regions.size();
}

void GRegionAjacencyGraph::averageColor(int nRegion, float* pRed, float* pGreen, float* pBlue)
{
...
int GRegionAjacencyGraph::ajacencyCount()
{
	return (int)m_neighbors.size();
}

void GRegionAjacencyGraph::ajacency(int nEdge, int* pRegion1, int* pRegion2)
{
...
void GSubImageFinder2::findSubImage(int* pOutX, int* pOutY, GImage* pNeedle, GRect* pNeedleRect)
{
	...
	for(size_t iters = 0; true; iters++)
	{
		...
		unsigned int n = pNeedle->pixel(pCoords[0], pCoords[1]);
		for(vector<GSIFStats*>::iterator it = cands.begin(); it != cands.end(); it++)
		{
			GSIFStats* pStats = *it;
			size_t* pCoords = cvi.current();
			unsigned int h = m_pHaystack->pixel(pStats->m_x + pCoords[0], pStats->m_y + pCoords[1]);
			int dif;
			dif = gRed(h) - gRed(n);
		...
		if(iters >= 64)
		{
			size_t thresh = MAX((size_t)10, best) * 4; // 2 squared (since we're comparing squared error)
			for(size_t i = 0; i < cands.size(); i++)
			{
				GSIFStats* pStats = cands[i];
				if(pStats->m_sse >= thresh)
				{
