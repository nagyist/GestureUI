...
/*
	...
	double alpha = GMath::Integrate(GData_PairedTTestHelper, -t, t, 200000, &dv);
...
// static
void GMath::pascalsTriangle(int* pOutRow, int nRow)
{
	GAssert(nRow >= 0);
	int i, j;
	for(i = 0; i <= nRow; i++)
...
void GMath::test()
{
	...
	if(ABS(dTarget - dComputed) > .00001)
		throw "wrong answer";
	...
	if(ABS(x[0] + x[1] * t0 + x[2] * t0 * t0 + x[3] * t0 * t0 * t0 - x0) > .0000001)
		throw "wrong answer";
	if(ABS(x[0] + x[1] * t1 + x[2] * t1 * t1 + x[3] * t1 * t1 * t1 - x1) > .0000001)
		throw "wrong answer";
	if(ABS(x[0] + x[1] * t2 + x[2] * t2 * t2 + x[3] * t2 * t2 * t2 - x2) > .0000001)
		throw "wrong answer";
	if(ABS(x[0] + x[1] * t3 + x[2] * t3 * t3 + x[3] * t3 * t3 * t3 - x3) > .0000001)
		throw "wrong answer";
}
#endif // !NO_TEST_CODE
