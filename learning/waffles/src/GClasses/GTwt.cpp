		...
		else if(*pSrc == '\\')
		{
			...
			if(*pSrc == 'n')
				...
				*pDest = '\\';
				...
				ThrowError("Unrecognized escape sequence \"\\", gformat(*pSrc), "\" at line ", gformat(line));
		}
		else
			*pDest = *pSrc;
		pDest++;
...
GTwtNode* GTwtDoc::parseLine(int level, char** pOutLabel)
{
	...
	if(*m_pDoc < '0' || *m_pDoc > '9')
		ThrowError("Expected a level number at the start of line ", gformat(m_line));
	int lev = atoi(m_pDoc);
	if(lev < level)
		return NULL;
	if(lev != level)
		ThrowError("Invalid level at the start of line ", gformat(m_line), ". Expected a value <= ", gformat(level));
	...
	if(m_len <= 1 || *m_pDoc != ' ')
		ThrowError("Expected more on line ", gformat(m_line));
	m_pDoc++;
	m_len--;
	...
	if(pOutLabel)
	{
		...
		if(i >= m_len || m_pDoc[i] != ':')
			ThrowError("Expected a ':' on line ", gformat(m_line));
	...
	switch(*m_pDoc)
	{
		...
		case 'l':
			{
				...
				if(m_len <= 0 || *m_pDoc < '0' || *m_pDoc > '9')
					ThrowError("Expected an item count on line ", gformat(m_line));
#ifdef WIN32
				...
				for(size_t i = 0; i < itemCount; i++)
				{
					...
					if(!pItem)
						ThrowError("Not enough items in list at line ", gformat(m_line));
					pNewObj->setItem(i, pItem);
				}
				if(parseLine(level + 1, NULL))
					ThrowError("Too many items in list at line ", gformat(m_line));
			}
			break;
		case 'b':
			m_pDoc += 2;
			m_len -= 2;
			if(m_len <= 0)
				ThrowError("Expected a 't' or 'f' on line ", gformat(m_line));
		...
		case 'i':
			...
			if(m_len <= 0 || (*m_pDoc != '-' && (*m_pDoc < '0' || *m_pDoc > '9')))
				ThrowError("Expected an integer value on line ", gformat(m_line));
#ifdef WIN32
		...
		case 'd':
			...
			if(m_len <= 0 || (*m_pDoc != '-' && *m_pDoc != '.' && (*m_pDoc < '0' || *m_pDoc > '9')))
				ThrowError("Expected a double value on line ", gformat(m_line));
		...
		case 's':
			...
			break;
		default:
			ThrowError("unexpected type '", gformat(*m_pDoc), "' at line ", gformat(m_line), ". Expected 'o', 'l', 'b', 'i', 'd', or 's'");
	}
	return pNewObj;
}
