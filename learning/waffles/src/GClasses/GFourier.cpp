...
bool GFourier::fft2d(int nArrayWidth, int nArrayHeight, struct ComplexNumber* p2DComplexNumberArray, bool bForward)
{
	...
	double* pTmpArray = new double[MAX(nArrayWidth, nArrayHeight) << 1];
...
void GFourier::test()
{
	...
	if(ABS(cn[0].real - 4) > 1e-12)
		throw "wrong answer";
	if(ABS(cn[0].imag) > 1e-12)
		throw "wrong answer";
	int n;
	for(n = 1; n < 3; n++)
	{
		if(ABS(cn[n].real) > 1e-12)
			throw "wrong answer";
		if(ABS(cn[n].imag) > 1e-12)
			throw "wrong answer";
	}
	GFourier::fft(4, cn, false);
	for(n = 0; n < 3; n++)
	{
		if(ABS(cn[n].real - 1) > 1e-12)
			throw "wrong answer";
		if(ABS(cn[n].imag) > 1e-12)
			throw "wrong answer";
	}
