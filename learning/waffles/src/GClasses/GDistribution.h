...
public:
	...
	void setValues(int nValueCount, const double* pValues);

	/// Sets the specified values, and normalizes
	void setValues(int nValueCount, int* pValues);

	/// Set all uniform probabilities
	void setToUniform(int nValues)
	{
		values(nValues);
		int i;
		for(i = 0; i < m_nValueCount; i++)
	...
	void setSpike(int nValueCount, int nValue, int nDepth);
...
protected:
	...
	double* m_pVector1;
	double* m_pVector2;
	GData* m_pInverseCovariance;
	GData* m_pCholesky;

public:
	GMultivariateNormalDistribution(const double* pMean, GData* pCovariance);
	GMultivariateNormalDistribution(GData* pData, int nDims);
...
protected:
	void precompute(GData* pCovariance);
};
