...
void GGraphCut::getEdgesFromRegionList(GRegionAjacencyGraph* pRegionList)
{
	...
	int i;
	for(i = 0; i < pRegionList->ajacencyCount(); i++)
	{
...
{
	GAssert(nNode >= 0 && nNode < m_pGraph->nodeCount()); // out of range
	m_nNode = nNode;
	m_pCurrentEdge = m_pGraph->m_pNodes[nNode].GetFirstEdge();
}

bool GGraphEdgeIterator::next(int* pNode, float* pEdgeWeight, bool* pOutgoing)
{
	if(!m_pCurrentEdge)
		return false;
...
{
	m_nodes = nodes;
	m_pCosts = new GData(nodes);
	m_pCosts->newRows(nodes);
...
GData* GFloydWarshall::releaseCostMatrix()
{
	m_pCosts = NULL;
	return m_pCosts;
}
...
void GFloydWarshall::test()
{
	...
	for(int i = 0; i < 100; i++)
	{
		...
		int edgeCount = (int)prng.next(NODE_COUNT * NODE_COUNT);
		for(int j = 0; j < edgeCount; j++)
		{
			int a = (int)prng.next(NODE_COUNT);
			int b = (int)prng.next(NODE_COUNT);
		...
		g1.compute();
		int origin = (int)prng.next(NODE_COUNT);
		g2.compute(origin);
		for(int j = 0; j < NODE_COUNT; j++)
		{
			double floyd = g1.cost(origin, j);
			double dijkstra = g2.cost(j);
			if(ABS(floyd - dijkstra) > 1e-8)
				ThrowError("wrong");
		}
	}
}
...
void GBrandesBetweennessCentrality::test()
{
	...
	graph.compute();
	if(ABS(graph.edgeBetweenness(0, 0) - 1) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(0, 1) - 4) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(2, 0) - 4) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(2, 1) - 4) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(2, 2) - 9) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(5, 0) - 1) > 1e-5)
		ThrowError("failed");
	if(ABS(graph.edgeBetweenness(5, 1) - 4) > 1e-5)
		ThrowError("failed");
}
#endif
