...
public:
	GKernelIdentity(int dims) : GKernel(), m_dims(dims) {}
...
public:
	GKernelPolynomial(int dims, double offset, unsigned int order) : GKernel(), m_dims(dims), m_offset(ABS(offset)), m_order(order) {}
...
public:
	GKernelGaussianRBF(int dims, double variance) : GKernel(), m_dims(dims), m_variance(ABS(variance)) {}
...
public:
	...
	GKernelTranslate(GKernel* pK, double value) : GKernel(), m_pK(pK), m_value(ABS(value)) {}
...
public:
	/// Takes ownership of pK
	GKernelScale(GKernel* pK, double value) : GKernel(), m_pK(pK), m_value(ABS(value)) {}
...
/// An experimental kernel machine. Currently this algorithm is not competitive with state-of-the-art kernel machines.
class GKernelMachine : public GSupervisedLearner
{
protected:
	int m_featureDims, m_labelDims;
	GKernel* m_pKernel;
	double* m_pWeights;
	GData* m_pSupportVectors;
	double* m_pBuf;

public:
	GKernelMachine();
	GKernelMachine(GTwtNode* pLearner);
	virtual ~GKernelMachine();

	/// Generates a big kernel by combining several simple kernels
	static GKernel* makeKernel(int dims);
	...
	double* swapWeights(double* pNewWeights);
};
