...
{
	double a, b, z;
	int yBegin = MAX(0, (int)floor(MAX(pBotLeft->m_vals[1], pBotRight->m_vals[1])));
	int yEnd = MIN((int)pImage->height(), (int)floor(MIN(pTopLeft->m_vals[1], pTopRight->m_vals[1])) + (pTopLeft == pTopRight ? 1 : 0));
	for(int y = yBegin; y < yEnd; y++)
	{
		...
		if(b < a)
			std::swap(a, b);
		int xBegin = MAX(0, (int)floor(a));
		int xEnd = MIN((int)pImage->width() - 1, (int)ceil(b));
