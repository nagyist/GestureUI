...
{
	float fontSize = MAX(1.0f, (float)h / 24);
...
void GWidgetCommon::drawClipped(GImage* pCanvas, int x, int y, GWidget* pWidget, GRect* pClipRect)
{
	...
	if((int)m_bufferImage.width() < pWidgetRect->w || (int)m_bufferImage.height() < pWidgetRect->h)
		m_bufferImage.setSize(MAX((int)m_bufferImage.width(), pWidgetRect->w), MAX((int)m_bufferImage.height(), pWidgetRect->h));

	int xx = MAX(x, pClipRect->x);
	int yy = MAX(y, pClipRect->y);
	GRect r(xx - x, yy - y, MIN(x + pWidgetRect->w, pClipRect->x + pClipRect->w) - xx, MIN(y + pWidgetRect->h, pClipRect->y + pClipRect->h) - yy);
...
{
	GRect r(0, 0, MIN((unsigned int)m_rect.w, pImage->width()), MIN((unsigned int)m_rect.h, pImage->height()));
...
void GWidgetTextBox::SetSelection(int anchorPos, int cursorPos)
{
	...
	m_nAnchorPos = MAX(0, MIN((int)m_text.length(), anchorPos));
	m_nCursorPos = MAX(0, MIN((int)m_text.length(), cursorPos));
}
...
void GWidgetGrid::setColumnCount(int n)
{
	...
	int nScrollBarSize = 16;
	int nDefaultColumnWidth = MAX((m_rect.w - nScrollBarSize) / n, 50);
...
void GWidgetFileSystemBrowser::reloadFileList()
{
	...
	int cols = MAX(1, MIN((m_rect.w - nScrollBarSize) / MIN_FILES_BROWSER_COL_WIDTH, (totalSize + displayrows - 1) / displayrows));
...
void GWidgetCanvas::setZoom(float f)
{
	...
	if(m_pImageIn)
	{
		if(m_nHScroll > (int)(m_fZoom * m_pImageIn->width()) - m_rect.w)
			m_nHScroll = MAX(0, (int)(m_fZoom * m_pImageIn->width()) - m_rect.w);
		if(m_nVScroll > (int)(m_fZoom * m_pImageIn->height()) - m_rect.h)
			m_nVScroll = MAX(0, (int)(m_fZoom * m_pImageIn->height()) - m_rect.h);
	}
	m_pParent->tattle(this);
}
...
{
	setZoom(MIN((float)m_rect.w / m_pImageIn->width(), (float)m_rect.h / m_pImageIn->height()));
}
...
void GWidgetWave::draw(GImage* pCanvas, int xx, int yy)
{
	...
	else if(m_pWave->bitsPerSample() == 8)
	{
		...
		ThrowError("Sorry, GWidgetWave doesn't support ", gformat(m_pWave->bitsPerSample()), " bit samples. It only supports 16 and 32 bit samples");
}
