...
void GRayTraceRay::ComputeRandomDiffuseVector(G3DVector* pOutVector)
{
	G3DVector u;
	G3DVector v;
	if(ABS(m_normalVector.m_vals[0]) < .5) // if N is not co-linear with (1, 0, 0)
	{
		v.set(1, 0, 0);
...
/*virtual*/ void GRayTraceDirectionalLight::colorContribution(GRayTraceScene* pScene, GRayTraceRay* pRay, GRayTraceMaterial* pMaterial, bool bSpecular)
{
	...
	diffuse.multiply(MAX((G3DReal)0, direction.dotProduct(&pRay->m_normalVector)));

	// Compute specular component of the color
	if(bSpecular)
	{
		G3DReal mag = (G3DReal)pow(MAX((G3DReal)0, pRay->m_reflectionVector.dotProduct(&direction)), pMaterial->specularExponent());
...
/*virtual*/ void GRayTracePointLight::colorContribution(GRayTraceScene* pScene, GRayTraceRay* pRay, GRayTraceMaterial* pMaterial, bool bSpecular)
{
	...
	// Compute diffuse component of the color
	GRayTraceColor diffuse(pMaterial->color(GRayTraceMaterial::Diffuse, pRay));
	diffuse.multiply(MAX((G3DReal)0, lightDirection.dotProduct(&pRay->m_normalVector)));

	// Compute specular component of the color
	if(bSpecular)
	{
		G3DReal mag = (G3DReal)pow(MAX((G3DReal)0, pRay->m_reflectionVector.dotProduct(&lightDirection)), pMaterial->specularExponent());
		GRayTraceColor specular(pMaterial->color(GRayTraceMaterial::Specular, pRay));
		specular.multiply(mag);
		diffuse.add(&specular);
	}
...
/*virtual*/ void GRayTraceAreaLight::colorContribution(GRayTraceScene* pScene, GRayTraceRay* pRay, GRayTraceMaterial* pMaterial, bool bSpecular)
{
	...
	if(m_pObject->type() == GRayTraceObject::Triangle)
	{
		...
		diffuse.multiply(ABS(pRay->m_normalVector.dotProduct(&lightDirection) * triangleNormal.dotProduct(&lightDirection) * triangleArea / (G3DReal)(M_PI * distsqared)));
	}
	else
	{
		GRayTraceSphere* pSphere = (GRayTraceSphere*)m_pObject;
...
bool GRayTracePhysicalMaterial::isSame(GRayTraceMaterial* pOther)
{
	...
	for(int i = 0; i < Color_Type_Count; i++)
	{
		if(ABS(pThat->m_colors[i].r - m_colors[i].r) > 1e-6)
			return false;
		if(ABS(pThat->m_colors[i].g - m_colors[i].g) > 1e-6)
			return false;
		if(ABS(pThat->m_colors[i].b - m_colors[i].b) > 1e-6)
			return false;
	}
	if(ABS(pThat->m_indexOfRefraction - m_indexOfRefraction) > 1e-6)
		return false;
	if(ABS(pThat->m_specularExponent - m_specularExponent) > 1e-6)
		return false;
	if(ABS(pThat->m_glossiness - m_glossiness) > 1e-6)
		return false;
	if(ABS(pThat->m_cloudiness - m_cloudiness) > 1e-6)
		return false;
	return true;
}
...
void GRayTracePhysicalMaterial::setColor(ColorType eType, G3DReal r, G3DReal g, G3DReal b)
{
	if(eType == Emissive)
		...
		m_colors[eType].set(1,
				MAX((G3DReal)0, MIN((G3DReal)1, r)),
				MAX((G3DReal)0, MIN((G3DReal)1, g)),
				MAX((G3DReal)0, MIN((G3DReal)1, b))
			);
}
...
GRayTraceColor* GRayTraceImageTexture::color(ColorType eType, GRayTraceRay* pRay)
{
	if(eType == Diffuse)
	{
		if(m_pTextureImage)
		{
			...
			x *= m_pTextureImage->width();
			y *= m_pTextureImage->height();
