	...
	while(true)
	{
		if(m_pSocket->GetMessageCount() > 0)
		{
			...
			if(blobSize < (int)sizeof(unsigned int))
				ThrowError("Client received tiny blob of size: ", gformat(blobSize));
			...
			else if(packetID == 104) // all done, please exit
			{
				*allDone = true;
				...
				ThrowError("Client received unexpected packet with id: ", gformat(packetID));
		}
...
bool GLoadBalanceMaster::results(GBlobIncoming* pBlobIn)
{
	...
	while(true)
	{
		...
		switch(packetID)
		{
			...
			default:
				ThrowError("Server received unexpected packet with id: ", gformat(packetID));
				break;
		}
	}
}
