...
public:
	...
	: GWidgetDialog(w, h, 0xff90d0f0) // a=ff, r=90, g=d0, b=f0
	{
		...
		m_pTrainingData = new GData(5);
		m_pTrainingData->newRows(m_pImageSmall->width() * m_pImageSmall->height());
		size_t ranges[2];
		...
		{
			double* pRow = m_pTrainingData->row(i);
			cvi.currentNormalized(pRow);
			pRow[2] = ((double)gRed(*pPix)) / 255;
			pRow[3] = ((double)gGreen(*pPix)) / 255;
			pRow[4] = ((double)gBlue(*pPix)) / 255;
			if(!cvi.advance())
				break;
			pPix++;
		}
	...
	void doModel(GTransducer* pModel)
	{
		...
		GData dataUnlabeled(5);
		dataUnlabeled.newRows(m_pImageBig->width() * m_pImageBig->height());
		size_t ranges[2];
		...
		for(size_t i = 0; true; i++)
		{
			double* pRow = dataUnlabeled.row(i);
			cvi.currentNormalized(pRow);
			if(!cvi.advance())
				break;
		}
		...
		// Predict labels
		pModel->transduce(m_pTrainingData, &dataUnlabeled, 3);
		...
		{
			double* pRow = dataUnlabeled.row(i);
			int r = ClipChan((int)(pRow[2] * 255.0));
			int g = ClipChan((int)(pRow[3] * 255.0));
			int b = ClipChan((int)(pRow[4] * 255.0));
	...
	void doBackProp(GNeuralNet* pNN)
	{
		...
		m_pNN->enableIncrementalLearning(pRel, 3, NULL, NULL);
		delete(m_pNN2);
	...
	void addModels()
	{
		...
		addModel("Naive Bayes, 10 discretized buckets"); // 7
		...
		addModel("NeuralNet 2-16-60-3, piecewise logistic"); // 16
	}
	...
	void onChoice(int index)
	{
		...
		else if(index == 7)
		{
			...
			GFilter filter(&model, false);
			filter.setFeatureTransform(new GDiscretize(10), false);
			filter.setLabelTransform(new GDiscretize(10), false);
			doModel(&filter);
		}
		else if(index == 8)
		{
		...
		{
			GBaselineLearner model(m_pRand);
			doModel(&model);
		}
		else if(index == 16)
		{
	...
	void workerThread()
	{
		while(m_workerMode > 0)
		{
			if(m_workerMode == 3) // train
			{
				...
				{
					double* pRow = m_pTrainingData->row((size_t)m_pRand->next(m_pTrainingData->rows()));
					m_pNN->trainIncremental(pRow, pRow + 2);
				}
			}
