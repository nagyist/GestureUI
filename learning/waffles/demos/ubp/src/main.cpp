...
public:
	UBPModel()
	: m_tar(5), m_context(2), m_pred(5), m_rand(0), m_nn(&m_rand), m_errorThresh(0.0), m_targetActive(1.0)
	{
		{
			GData* pTmp = GData::loadArff("in.arff");
		...
		m_nn.setBackPropTargetFunction(GNeuralNet::root_cubed_error);
		m_nn.addLayer(36);
		m_nn.setLearningRate(0.1);
		m_nn.enableIncrementalLearning(pRel, 3, NULL, NULL);
...
//		cout << "target active: " << m_targetActive << ", active: " << activeCount << ", thresh: " << m_errorThresh << "\n";
	...
	GData& pred() { return m_pred; }

	GData& tar() { return m_tar; }
	GData& context() { return m_context; }
};
...
public:
	...
	void makeCamera(GImage* pImage, GData* pDataTar, GData* pDataPred)
	{
		...
		m_pDataSorter = new GData(pDataTar->relation());
	...
	void makeImage(GImage* pImage, GData* pDataTar, GData* pDataPred)
	{
	...
	void makeImage2(GImage* pImage, GData* pData, GData* pDataPred)
	{
		pImage->clear(0xffffffff);
		if(m_viewRect.w > 0 && m_viewRect.h > 0)
		{
	...
	{
		m_yaw += dYaw;
		m_pitch = MAX(-M_PI / 2, MIN(M_PI / 2, m_pitch + dPit));
